describe('App loaded data properly with desktop set components and ', () => {
    beforeEach(() => {
    ...
    describe('should display properly all data in the card like ', () => {
        it('card title', () => {
            cy.get('[data-id=card]').eq(0).get('[data-id=desktop-card-title]').should('contain', 'MicroBooks');
        });
        ...
        it('establishment date and url', () => {
            cy.get('[data-id=card]').eq(0).get('[data-id=desktop-card-footer]').should('contain', '09.02.1995 - https://www.micro-books-store.com');
        });
